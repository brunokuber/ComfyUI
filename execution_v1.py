...
def execute_single_node(server, node_id, node_data, custom_inputs=None, state_manager=None):
    ...
    # 处理输入
    inputs = node_data["inputs"].copy()  # 浅拷贝输入
    if custom_inputs:
        # 合并自定义输入
    ...
    # 获取节点的输入定义和函数签名
    required_inputs = node_instance.INPUT_TYPES()["required"]
    print(f"Node {node_id} requires inputs: {list(required_inputs.keys())}")
    
    # 获取正确的函数名
    func_name = getattr(node_instance, "FUNCTION", "execute")
    func = getattr(node_instance, func_name)
    
    # 特殊处理CLIPTextEncode节点 - 它的函数参数顺序与输入定义不匹配
    ...
    for input_name in required_inputs:
        if input_name in inputs:
            ...
            print(f"Processing input {input_name}: {input_value} (type: {type(input_value).__name__})")
            ...
            if isinstance(input_value, list) and len(input_value) == 2:
                ...
                print(f"  Reference to node {ref_node_id} output {output_idx}")
                
                if state_manager:
                    if ref_node_id in state_manager.node_outputs:
    ...
    if class_type == "CLIPTextEncode":
        # CLIPTextEncode.encode(clip, text) 但输入定义是 text, clip
        input_args = [input_names_to_values['clip'], input_names_to_values['text']]
        print("Reordering CLIPTextEncode parameters to match (clip, text) signature")
    elif class_type == "VAEDecode":
        # VAEDecode.decode(vae, samples) 但输入定义是 samples, vae
        input_args = [input_names_to_values['vae'], input_names_to_values['samples']]
        print("Reordering VAEDecode parameters to match (vae, samples) signature")
    else:
        # 常规节点按输入定义顺序准备参数
        input_args = [input_names_to_values[name] for name in required_inputs]
    ...
    if class_type == "SaveImage" and "images" in input_names_to_values:
        ...
        if hasattr(images, "detach"):
            input_names_to_values["images"] = images.detach()
            print("Detaching tensor gradients for SaveImage node")
            # 更新input_args中对应位置的值
            img_index = list(required_inputs.keys()).index("images")
            input_args[img_index] = input_names_to_values["images"]
            
    # 验证参数
    print(f"Executing node {node_id} with {len(input_args)} inputs")
    for i, arg in enumerate(input_args):
        print(f"  Input {i}: {type(arg).__name__} at {id(arg)}")
